    ...
    return (
        <div>
            ...
            <div className={`cards-slider active-slide-${movie.id}`}>
                <div className="cards-slider-wrapper" style={{transform:`translateX(-${movie.id*(100/movieList.length)}%)`}}>
                    {
                        movieList.map(movie => <CardComponent key={movie.id} movie={movie}/>)
                    }
                </div>
            </div>
            <div className="position-absolute rate-card">
            <div style={{width:"100vw",height:"5px",background:"#EA2027"}}></div>
            <h2 className="p-3">LIST OF MOVIES:</h2>
                {
                    movieList.map(movie => <RateCardComponent key={movie.id} movie={movie}/>)
                }
            </div>
        </div>
    );
